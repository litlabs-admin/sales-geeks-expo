...
const responses = await Promise.all(
  Array.from({ length: 100 }, () => postScan({ token, qr, bypassRateLimit: true }))
);

if (responses.some((response) => ![200].includes(response.status))) {
  throw new Error(`Expected all parallel scan responses to be 200, saw ${responses.map((r) => r.status).join(",")}`);
}
...
if (result?.scans !== 1 || result.competition_score !== qr.points || result.spendable_balance !== qr.points) {
  throw new Error(`Expected one award worth ${qr.points}, saw ${JSON.stringify(result)}`);
}

await closePhase5();
await closeSql();
console.log("Scan idempotency checks passed.");
