...
1. Every module in sections 3, 4, 5, and 6 is implemented and behaves as specified.
2. The testing expectations in section 8 are met and the test suite is green in CI.
3. Every item in section 11 (Final Deliverables) is delivered.
4. The client has run an end-to-end rehearsal on staging and signed off.
5. Scottish Growth Expo 2026 has been operated on production using the system, and the post-event archive transition has been executed cleanly.
6. The post-event analytics report has been delivered to SalesGeek Scotland.
...
## 14. Questions for the Expo Event Manager

Things to ask the event manager before build starts in earnest. Answers here unblock entire modules — getting them upfront avoids rework.

### 14.1 Event Basics & Branding
1. What is the **exact event start time, end time, and prize scoring deadline** for Scottish Growth Expo 2026?
2. Is the canonical public name confirmed as *Scottish Growth Expo 2026* with *SalesGeek Scotland* as host/powered-by, or are there variations to support?
3. What is the **agreed app domain** (e.g. `app.salesgeek.scot`) and the event slug to use?
4. Do you have a finalized **brand kit** for the event — logos, colors, typography, hero imagery — or do we use SalesGeek Scotland's existing identity as-is?
5. Should the Forumm public listing keep a link out to the companion app, or do you want attendees to find it only via QR/email?

### 14.2 Venue & Logistics
6. What is the **final venue floorplan** at Hampden, and do we have a digital asset for it (image, PDF, or coordinates)?
7. What are the **broad venue zones** you want surfaced in the app (e.g. main stage, breakout rooms, sponsor hall, networking zone)?
8. Where exactly will the **registration desk** be, and how many staff will run it during peak arrival?
9. What is the **expected attendee count**, peak arrival window, and any other traffic-shape information that affects load planning?
10. What is the **venue's Wi-Fi situation** — is there a guest network, what's the realistic bandwidth/quality, and is there cellular reception at all zones?

### 14.3 Agenda & Speakers
11. Is the **full agenda** locked, and by what date? We need it loaded for pre-event mode.
12. Who are the **confirmed speakers**, and do you have bios, headshots, and any session-level sponsor associations?
13. Which sessions get **session QRs for attendance points**, and what are the active windows (typically session start ± how many minutes)?
14. Are there any **non-session moments** (keynote intros, panels, networking blocks) that should appear on the agenda?

### 14.4 Hosts & The William Reward
15. Confirmed bios, photos, contact links, and booking links for **all four host SalesGeeks**?
16. For William specifically: which Calendly link or event type backs the **premium post-event strategy session**?
17. **How many slots** of the William strategy session should be made available as a reward?
18. What's William's **availability window** for those sessions after the event?
19. Do you want William's premium reward to be visible to all attendees from day one, or unlocked at a certain leaderboard threshold/score?

### 14.5 Sponsors
20. Final **sponsor list with tiers** — and any tier-specific behavior in the app (e.g. higher tier = more prominent placement)?
21. For each sponsor, who provides **logo, blurb, website URL, and any imagery**?
22. Each sponsor's **physical booth location** at Hampden (zone) for QR placement and venue references?
23. Have sponsors **agreed to the lead-sharing model** — that they get scans and interest signals only when the attendee consents?
24. What is each sponsor's **point value** for their main QR, and do any sponsors get hidden/secondary QRs (e.g. swag pickup)?
25. **Sponsor reporting cadence** — do they want a live link/feed, or a post-event report only?

### 14.6 Rewards Catalog
26. Full list of **physical rewards** with quantities, costs (in points), per-attendee limits, expiry, and any sponsor association?
27. Where are physical rewards **collected from**, and who staffs that desk?
28. Any **staff-only rewards** vs **self-service rewards** that the attendee should be able to claim entirely in-app?
29. Should rewards become **available all day**, or are some time-gated (e.g. evening-only)?
30. Is there a **headline reward** beyond the William session, and what does winning the top-of-leaderboard prize involve operationally?

### 14.7 QR Strategy
31. How many **sponsor QRs**, **session QRs**, and **hidden bonus QRs** should the event have in total?
32. For hidden bonus QRs: how many, what's the **reveal schedule** through the day, and which zones do they live in?
33. Do you want **printed posters/cards/badges** for QRs, and what format (A5, A4, table-tent, sticker) and finish?
34. Who is responsible for **physically placing** the QRs at the venue, and when (day before? morning of?)?
35. Are there any **off-stage moments** worth gamifying (e.g. attending a workshop, completing a survey, talking to the SalesGeek team) that should have their own QR?

### 14.8 Staff & Operations
36. **How many staff** will operate on event day, and how many need admin vs staff access?
37. **Names, emails, and roles** of every admin and staff account to be provisioned ahead of time?
38. Who is the **incident owner on the day** — the one person we call when something operationally critical happens?
39. Do you want a **pre-event dry run** at the venue itself, or only in staging?
40. What's the desired **support arrangement on the day** — on-site engineer, remote on-call, or both?

### 14.9 Communications & Notifications
41. Are there **scheduled announcements** known up front (e.g. "doors open", "keynote starts in 10", "lunch is served") that we should pre-load?
42. Who is **authorized to send live announcements** during the event?
43. Does the event email need a **custom from-address/domain** for OTP and booking confirmations, or do we use a SalesGeek inbox?
44. Are there **legal/privacy disclaimers** that need to live in the app (terms of use, privacy policy, sponsor consent text) — and is the wording approved?

### 14.10 Pre-Event Attendee Data
45. Will we get a **CSV of pre-registered attendees** to import, and in what format and timing?
46. What fields are **available pre-event** — name, email, phone, business, anything else?
47. Are pre-event attendees getting a **direct link to pre-register in the app**, or do we expect them to enter cold on event day?
48. Does the **Forumm ticketing data** flow to us in any form, or is the CSV the only source?

### 14.11 Post-Event
49. How long should the **post-event archive window** stay open for attendees? (PRD says 10 days — is that confirmed?)
50. Do you want a **post-event email to attendees** linking back to their archive view, and if so who sends it (us, you, or via your normal channels)?
51. What does the **post-event analytics report** need to contain for it to be useful to you and to sponsors?
52. Are there any **commitments to sponsors** about timeline of their lead exports we need to meet?

### 14.12 Legal & Compliance
53. Has the **prize competition** been checked against UK/Scottish promotional competition rules, and is there a T&Cs document we need to surface?
54. Are there any **age restrictions** on attendees or specific data handling we need to enforce?
55. Is **GDPR data export and deletion** something you want admin tooling for at launch, or can it be handled manually?

---

## 15. Assets & Inputs Needed from the Event Manager

A concrete checklist of what we need delivered to us, and when. Treat this as the input side of the project plan.

### 15.1 Brand & Visual Assets
- Event logo (SVG + PNG, light and dark variants).
- SalesGeek Scotland host/powered-by logo lockup.
- Brand color palette (hex codes) and typography (font files or web-font references).
- Hero/cover imagery for app surfaces.
- Sponsor logos (vector preferred, with usage guidelines if any).
- Speaker headshots (square crop preferred, minimum 800×800).
- Host Geek photos and short bios (under 500 chars each).
- Any iconography or illustration style the event uses.

### 15.2 Content
- Final agenda with session times, stages, descriptions, speaker links, sponsor links, and type/category.
- Speaker bios with contact/social links where relevant.
- Sponsor blurbs, website URLs, booth zones, and lead-sharing consent confirmation.
- FAQ content (initial set of 10–20 likely questions).
- Onboarding screen copy and "how points work" explainer copy.
- Terms of use, privacy policy, sponsor consent text — final and approved.
- Venue map / floorplan (image, PDF, or text-only directions if no map exists).
- Welcome message and any branding strings for the Home tab.

### 15.3 Event Configuration Inputs
- Final event start/end times and prize-scoring deadline.
- Venue zone list with names and short descriptions.
- Sponsor list with tiers, point values, booth zones.
- Reward catalog: name, cost, inventory, per-attendee limit, expiry, type (self-service vs staff-only), collection instructions.
- William reward specifics: slot count, Calendly link/event type, post-event availability window.
- Session-attendance QR list with active windows.
- Hidden bonus QR plan: count, reveal schedule, zone hints, point values.

### 15.4 People & Access
- Names, emails, and role (Admin vs Staff) for every operator that needs an account.
- Designated incident owner contact for event day.
- Sponsor primary contacts (for any reporting questions, though no sponsor logins in v1).
- Photographer/marketing contact (if event imagery feeds back into the app).

### 15.5 Third-Party Account Access
- Domain/DNS access (or instructions for SalesGeek's IT to point the agreed subdomain).
- Email-sending domain ownership confirmation (SPF/DKIM/DMARC setup) for OTP and booking emails.
- William's Calendly account access or, at minimum, the event-type URL and webhook configuration permissions.
- Hosting account ownership decisions (client-owned from day one vs transferred at handover).
- Any analytics tooling accounts SalesGeek wants the app instrumented for.

### 15.6 Pre-Event Data
- CSV of pre-registered attendees (with column mapping for name, email, phone, business).
- Forumm ticketing export if available, in any usable form.
- Any prior SalesGeek event mailing lists relevant to pre-event communications.

### 15.7 Approvals & Sign-Offs
- Final approval on the canonical event name and branding.
- Sign-off on the prize competition T&Cs.
- Sign-off on the sponsor consent flow wording.
- Sign-off on the staging UAT before going live.
- Sign-off on the event-day go/no-go checklist 24–48 hours before the event.

### 15.8 Timeline Asks
- **T-8 weeks**: brand assets, sponsor list, draft agenda, reward catalog draft, William reward specifics.
- **T-4 weeks**: final agenda, all sponsor content, final reward catalog, full QR plan, staff/admin account list, legal copy approved.
- **T-2 weeks**: pre-registered attendee CSV (first batch), printed QR assets approved, staging UAT complete.
- **T-1 week**: final CSV import, dry run at staging with client team, on-site logistics confirmed.
- **T-48h**: go/no-go checklist signed off, event switched into event-day mode in staging for final smoke test.
- **Event day**: production switched to event-day mode, on-call support active.
- **T+1 day**: post-event archive transition.
- **T+10 days**: attendee archive access closes (per PRD).
- **T+2 weeks**: post-event analytics report delivered.
